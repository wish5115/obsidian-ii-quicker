...
export default class IIPlugin extends Plugin {
	...
	registerQuickInsertEvent() {
		if (this.isMobile) {
			new QuickInsert(this).registerQuickInsertCommand();
		} else {
	...
	registerFileContextMenu() {
		this.registerEvent(
			this.app.workspace.on("file-menu", (menu, file) => {
				if(!this.settings.showFileCopyPathContextMenu) return
				menu.addItem((item) => {
					item.setTitle(t("Copy file path"))
						.setIcon("copy")
						.onClick(async () => {
							...
							if(basePath) {
								// @ts-ignore
								const fullPath = adapter?.path.join(basePath, file.path);
	...
	addSettingsMinimize(listen = true) {
		...
		const getSvgEl = () => {
			...
			const svgSelector2 =
				".side-dock-settings > .side-dock-ribbon-action:nth-child(3) > svg";
		...
		ready(async () => {
			...
			const addMinButton = () => {
				...
				if (!settings) return;
				const obMinBtn = document.querySelector(
					".ob-settings-minimize-btn"
				);
				if (obMinBtn) return;
				setTimeout(() => {
					const settings = document.querySelector(".mod-settings");
					...
					const newMinTips = createEl("span", {
						...
						text: t(
							"Note: During minimization, can't open modal windows, such as control panels."
						),
					});
					...
					newMinBtn.onclick = () => {
						//点击最小化按钮，把设置对话框隐藏
						settingsContainerDisplay = getComputedStyle(
							settings?.parentElement as Element,
							null
						).getPropertyValue("display");
						settings?.parentElement?.find(".modal-bg").hide();
						settings?.addClass("ii-ob-settings-hide");
						setTimeout(() => {
							settings?.parentElement?.setAttr(
								"style",
								"display:none;"
							);
			...
			this.obSettingBtnCallback = () => {
				...
				if (settings && settings?.parentElement) {
					const settingsStyle =
						settings?.parentElement.getAttr("style");
					if (
						settingsStyle &&
						settingsStyle?.indexOf("display:none") !== -1
					) {
						settings?.parentElement?.setAttr(
							"style",
							`display:${settingsContainerDisplay};`
						);
			...
			if (listen) {
				...
				this.boundObSettingBtnCallback =
					this.obSettingBtnCallback.bind(this);
				this.registerDomEvent(
					obSettingBtn as HTMLElement,
					"click",
					this.boundObSettingBtnCallback
				);
				...
				obSettingBtn.removeEventListener(
					"click",
					this.boundObSettingBtnCallback
				);
			}
